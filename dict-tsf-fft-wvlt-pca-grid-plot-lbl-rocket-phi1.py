...
# Configure logging
logging.basicConfig(filename='phi1.log', level=logging.INFO)
...
def read_file(file_path):
    try:
        return pd.read_csv(file_path, sep=r'\s+', header=None, usecols=[0], skiprows=1, encoding='utf-8')
    except UnicodeDecodeError:
        return pd.read_csv(file_path, sep=r'\s+', header=None, usecols=[0], skiprows=1, encoding='latin1')
...
def preprocess_batch(batch_data, hkey=1):
    ...
    flattened_phi1 = pd.DataFrame(batch_data)  # `batch_data` loaded from your source
    # flattened_phi1 = pd.DataFrame(data_normalized)
    print("Flattened phi1 shape:", flattened_phi1.shape)

    # Create a MultiIndex for (series, time) to treat each row as an independent series
    n_samples, n_timepoints = flattened_phi1.shape
    ...
    new_df = pd.DataFrame(flattened_phi1.values.flatten(), index=multiindex, columns=['phi1'])
    ...
    # Combine features using hstack directly -- try one of the combinations (benchmarks performed for phi_1)
...
def plot_clusters(data, clusters, method_name, n_pca=3):
    ...
    plt.savefig(f'{method_name}_clusters_phi_1.png')
    plt.close()
        ...
        for page in range(num_pages):
            ...
            for i in range(5):
                ...
                if idx < len(sample_indices):
                    ...
                    # Add horizontal grey lines at -180 and +180 degrees
                    axs[i].axhline(y=-180, color='grey', linestyle='--')
                    axs[i].axhline(y=180, color='grey', linestyle='--')
...
def main(directory_path, labels_folder, batch_size=22288, subset_size=22288):
    ...
    output_file = 'dynamic_map_phi_1.csv'
    ...
    for hkey in [4]: # range(1, len(combined_features)+1):
        ...
        # for umap_components, n_neighbors, min_dist in product(range(20, 160, 10), range(10, 80, 10), [0.0003125, 0.000625]):
        # for umap_components, n_neighbors, min_dist in product([170], [100], [0.000625]):

	# === GRID USED TO GENERATE TABLE 2 (phi1) ===
	for umap_components, n_neighbors, min_dist in product(
	    range(50, 160, 10),
	    range(10, 80, 10),
	    [0.000625]
	):
    ...
    output_file = f"{best_method}_dynamic_map_phi_1.csv"
    create_dynamic_map(directory_path, output_file, labels_folder, best_clusters, files)

    # === SAVE: EMBEDDING ÓTIMO DO PIPELINE (φ1) PARA ANÁLISE ORM ===
    best_embedding_path = os.path.join(labels_folder, "phi1_best_embedding.npy")
    best_filenames_path = os.path.join(labels_folder, "phi1_best_filenames.npy")
...
if __name__ == "__main__":
    # main("./saturn-orbits/origin", "./saturn-orbits/labels_phi_1")
    main("./orbits-dataset", "./orbits-dataset/labels_phi_1")
